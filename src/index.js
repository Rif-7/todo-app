...
import {
  ...
  initLocalStorage,
  setDemo,
} from "./app/localStorageFunctions";
import {
  loadProjectsToDom,
  setModal,
  loadNewTodoDetails,
  reloadTodos,
  loadTodaysTodosToDom,
  deleteProjectFromDom,
} from "./app/DomFunctions";
...
function makeChange(showDemo = true) {
  document.querySelector(".project-name").click();
  localStorage.clear();
  initLocalStorage(showDemo);
  loadProjectsToDom();
  reloadTodos();
  document.querySelector(".project-name").click();
}
...
document
  .querySelector(".today-todo")
  .addEventListener("click", loadTodaysTodosToDom);
document
